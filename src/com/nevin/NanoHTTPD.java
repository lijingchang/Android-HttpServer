...
public class NanoHTTPD
{
	...
	public Response serve( String uri, String method, Properties header, Properties parms, Properties files )
	{
		...
		while ( e.hasMoreElements())
		{
			...
			Log.i("rrrrrrr","  HDR: '" + value + "' = '" +header.getProperty( value ) + "'");
		}
		...
		while ( e.hasMoreElements())
		{
			...
			Log.i("rrrrrrr","  PRM: '" + value + "' = '" +parms.getProperty( value ) + "'");
		}
		...
		while ( e.hasMoreElements())
		{
			...
			Log.i("rrrrrrr","  UPLOADED: '" + value + "' = '" +files.getProperty( value ) + "'" );
		}
			...
			public void run(){
				if((myServerSocket==null)||(myServerSocket.isClosed())){
					try {
						...
						e.printStackTrace();
						Log.i("rrrrrrr","errorkill1--->"+e.toString());
					}
				}
				try{
					while( true )
					...
					Log.i("rrrrrrr","errorkill2--->"+ioe.toString());
				}
			}
		});
	...
	private class HTTPSession implements Runnable
	{
		...
		public void run()
		{
			...
			{
				Log.i("rrrrrrr","--error2--->"+ioe.toString());
				try
				{
		...
		private  void decodeHeader(BufferedReader in, Properties pre, Properties parms, Properties header)
			...
			try {
				...
				int b=inLine.indexOf("HTTP");
	//			if((a>0)&&(b>0))
	//			markcode=inLine.substring(a, b-1);
				markcode=Environment.getExternalStorageDirectory().toString();
			...
			boolean allowDirectoryListing )
	{
		...
		if ( res == null && f.isDirectory())
		{
			...
			if ( res == null )
			{
				...
				else if ( allowDirectoryListing && f.canRead() )
				{
					...
					msg += "<group id=\"videos_onHDP\" name=\"video_test\">";
					...
					if (files!=null)
					{
						for ( int i=0; i<files.length; ++i )
						{
							...
							msg +="<item name=\""+files[i]+"\" url=\"http://192.168.1.188:8070"+encodeUri( uri + files[i] )+"\"/>";
						}
					}
					msg += "</group>";
					res = new Response( HTTP_OK, MIME_XML, msg );
				}
				else
				{
					res = new Response( HTTP_FORBIDDEN, MIME_PLAINTEXT,
		...
		try
		{
			if ( res == null )
			{
				...
				long fileLen = f.length();//f.length();
				Log.i("rrrrrrr","len--0->"+fileLen+"---startfrom--->"+startFrom);
				if (range != null && startFrom >= 0)
				{
					...
					else
					{
						...
						Log.i("rrrrrrr","newLen-1-->"+newLen+"--endAt--1--->"+endAt);
						...
						res.addHeader( "Content-Length", "" + dataLen);//dataLen
				...
				{
					Log.i("rrrrrrr","etag-2-->"+etag);
					if (etag.equals(header.getProperty("if-none-match")))
						res = new Response( HTTP_NOTMODIFIED, mime, "");
					else
					{
						Log.i("rrrrrrr","etag-3-->"+etag);
						res = new Response( HTTP_OK, mime, new FileInputStream( f ));
						res.addHeader( "Content-Length", "" + fileLen);//fileLen
						res.addHeader( "ETag", etag);
					}
				}
			}
		...
		catch( IOException ioe )
		{
			...
			Log.i("rrrrrrr","error-1--"+ioe.toString());
		}
